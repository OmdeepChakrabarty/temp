...
export const basicFactory: CapabilityFactory<BasicParams> = {
  prepare: async (params: BasicParams, ports, ctx: PrepareContext) => {
    ...
    const patchedSource = installExtensions({ vertexShader: '', fragmentShader: '' }, [{ id: params.kind ?? 'basic', version: 1, uniforms: [{ name: params.name ?? 'uValue', type: (params.uniformType as 'float'|'vec2'|'vec3'|'vec4'|'color'|'int') ?? 'float', default: params.defaultValue ?? 0 }], hooks: ['vertex-position', 'surface-color', 'alpha-discard', 'emissive'] }]);
    const programKey = makeProgramKey('three-r186', patchedSource, {});
    const handle: TypedHandle = { kind: 'material-extension', version: 1, value: { spec, source: patchedSource, key: programKey, shadowVariant: createShadowVariants({ customDepthMaterial: null, customDistanceMaterial: null }), uniformChannels: [{ name: params.name ?? 'uValue', type: (params.uniformType as 'float'|'vec2'|'vec3'|'vec4'|'color'|'int') ?? 'float' }], deterministicSource: true }, ownership: 'borrowed', features: ['uniform', 'extension-spec', 'source-generated', 'program-key-stable', 'glsl-patch-applied'] };
